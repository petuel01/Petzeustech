...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ user }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      console.error("Critical Node Sync Error");
    }
  };
    ...
    <div className="max-w-7xl mx-auto px-6 py-12 flex flex-col gap-10 animate-fade-in">
      <div className="flex flex-col md:flex-row justify-between items-end gap-6 border-b border-white/5 pb-8">
        ...
        <div className="flex gap-4">
          {['payments', 'files', 'users'].map((tab: any) => (
            <button 
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-blue-600 text-white shadow-xl shadow-blue-500/20' : 'bg-white/5 text-slate-500 hover:text-white'}`}
            >
              {tab}
            </button>
          ))}
              ...
              <tbody className="divide-y divide-white/5">
                {pendingPayments.map((p) => (
                  <tr key={p.id} className="hover:bg-white/5 transition-colors group">
                    ...
                    <td className="p-8"><span className="px-3 py-1 bg-blue-600/10 text-blue-400 rounded-lg text-[10px] font-black uppercase tracking-widest border border-blue-500/20">{p.planName}</span></td>
                    ...
                    <td className="p-8 text-right">
                      <div className="flex justify-end gap-3">
                        <button onClick={() => processPayment(p.id, 'APPROVE')} className="bg-emerald-600/10 text-emerald-500 hover:bg-emerald-600 hover:text-white px-5 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest border border-emerald-500/20 transition-all">Approve</button>
                        <button onClick={() => processPayment(p.id, 'REJECT')} className="bg-red-600/10 text-red-500 hover:bg-red-600 hover:text-white px-5 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest border border-red-500/20 transition-all">Decline</button>
                      </div>
                    </td>
                  </tr>
                ))}
                {pendingPayments.length === 0 && (
                  <tr><td colSpan={5} className="p-20 text-center text-slate-600 font-black uppercase tracking-widest text-xs">No pending transactions in cluster.</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {activeTab === 'files' && (
        <div className="max-w-2xl mx-auto w-full bg-[#0f172a] p-12 rounded-[3rem] border-2 border-white/5 shadow-3xl text-center">
          <h2 className="text-3xl font-black text-white uppercase tracking-tighter mb-4">BROADCAST PAYLOAD</h2>
          <p className="text-slate-500 text-[10px] font-bold uppercase tracking-[0.2em] mb-12 leading-relaxed">ASSOCIATE SECURE .SIP CONFIGURATIONS WITH TARGET SUBSCRIPTION NODES.</p>
          
          <form onSubmit={handleFileUpload} className="flex flex-col gap-8">
            <div className="flex flex-col gap-3 text-left">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-4">Select Target Tier</label>
              <select 
              ...
              >
                ...
                <option value="master">Architect Master File</option>
              </select>
            </div>
            ...
            <div className="flex flex-col gap-3 text-left">
              ...
              >
                ...
                {fileForm.fileName ? fileForm.fileName.name : 'Drop .sip payload or Click to Select'}
              </label>
            </div>
            ...
            >
              {isLoading ? 'BROADCASTING TO CLUSTER...' : 'INITIATE TIER BROADCAST'}
            </button>
          </form>
        </div>
      )}
      ...
      {activeTab === 'users' && (
        <div className="bg-[#0f172a] rounded-[3rem] border-2 border-white/5 overflow-hidden shadow-3xl">
          <div className="p-10 border-b border-white/5">
            <h3 className="text-xl font-black text-white uppercase tracking-widest">Network Map (Active Nodes)</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-white/5 bg-slate-950/50">
                  ...
                  <th className="p-8 text-[10px] font-black text-slate-500 uppercase tracking-widest">Node Status</th>
                  <th className="p-8 text-right text-[10px] font-black text-slate-500 uppercase tracking-widest">Joined</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5">
                {allUsers.map((u) => (
                  <tr key={u.id} className="hover:bg-white/5 transition-colors">
                    <td className="p-8">
                      ...
                      <p className="text-[9px] text-slate-500 mt-1">{u.email}</p>
                    </td>
                    <td className="p-8">
                    ...
                    <td className="p-8 text-right text-[10px] font-mono text-slate-500">{u.created_at || 'ARCHIVAL DATA'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
